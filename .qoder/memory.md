...
每个实验脚本末尾（代码之后）加一段注释块「执行逻辑与验证」：
- **执行逻辑**：按执行顺序分步（主流程怎么跑、各实验/函数依次做什么）
- **验证内容**：看哪些输出/断言判断实验成功（预期现象 + 验收标准）
- 阅读顺序：读者先看代码，再回头看文末这段——理解成本最低；随示例复杂度升高尤其必要
- 自 `10_kv_cache_pipeline.py` 起已回填；后续新脚本默认携带
