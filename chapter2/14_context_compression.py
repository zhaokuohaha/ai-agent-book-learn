...
# ---- 执行逻辑与验证（看完代码再回头看这里） ----
# 执行逻辑：
#   1. 主循环 run(model, compressor, ...)：每轮先 maybe_compress（压缩插在
#      两次 API 调用之间，不是调用中）→ 算总字符，压完仍超 WINDOW 才判溢出
#      （给压缩一次机会）→ invoke 全量重发 → 无 tool_calls 收工；有则执行
#      工具追加轨迹回到开头
#   2. maybe_compress：受害者 = 除最后 KEEP_RECENT 条外未标记的 tool 消息
#      （[COMPRESSED] 前缀防重复）→ 阈值门（攒到 80% 窗口才批量压，不每轮压）
#      → 裸模型逐条摘要、原地替换（条数不变，tool_call_id 配对链不破）
#      → commonprefix 量出仍可命中的缓存前缀
#   3. 对照组/实验组同任务同窗口，唯一变量 compress_enabled；两个模型两种
#      身份：base 裸模型当压缩器，bind_tools 版当决策模型（菜单决定行为面）
# 验证内容：
#   - 无压缩组 4 家明细 ~2746 字符 > 1500 → 溢出失败（原书：无压缩 5 轮爆窗）
#   - 压缩组 2746 → ~900 完成，四家统计全对（检索摘要结论，不再逐笼重数）
#   - 每条 [摘要] 打印可见；公共前缀止于首个替换点（前缀缓存仍命中的字节证据）
#   - 踩坑（详见文件头留档）：压缩器若绑了工具会"先查一下"→ 空摘要 → 数据缺失
